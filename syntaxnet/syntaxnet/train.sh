...
MORPH_HIDDEN_LAYER_SIZES=128
MORPH_HIDDEN_LAYER_PARAMS=128
GREEDY_MORPH_PARAMS=${MORPH_HIDDEN_LAYER_PARAMS}-0.08-3600-0.9-0

function train_morpher {
    ${BINDIR}/parser_trainer \
      ...
      --decay_steps=3600 \
      --hidden_layer_sizes=${MORPH_HIDDEN_LAYER_SIZES} \
      --learning_rate=0.08 \
      --momentum=0.9 \
      --beam_size=1 \
      --seed=0 \
      --params=${GREEDY_MORPH_PARAMS} \
      --num_epochs=12 \
      --report_every=10 \
...
function copy_model {
  ...
  cp -rf ${TMP_DIR}/brain_morpher/greedy/${GREEDY_MORPH_PARAMS}/char-ngram-map ${MODEL_DIR}/
  cp -rf ${TMP_DIR}/brain_morpher/greedy/${GREEDY_MORPH_PARAMS}/morph-label-set ${MODEL_DIR}/
  cp -rf ${TMP_DIR}/brain_morpher/greedy/${GREEDY_MORPH_PARAMS}/morphology-map ${MODEL_DIR}/
}
